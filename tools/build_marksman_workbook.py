...
OUT_PATH = REPO / "Marksman-DPS-Calculator.xlsx"
...
def build_calc_sheet(wb):
    ...
    for key, r in ROW.items():
        ...
        if key == "EMPOWERED_PIERCING_ARROW":
            ...
            ws.cell(row=r, column=15, value=(
                f'=IF(C{r},0.15*{bolt_surplus_trigger_rate}*{S("HitsPerCast", r)}*N{r}*'
                f"{target_multiplier_expr(IB('monster_type'), IB('normal_weight_frac'), S('NormalMonsterTargets', r), IB('max_enemies_hit'))},0)"
            ))
        else:
        ...
        if key in BOLT_SURPLUS_ROW_KEYS:
...
def build_stat_block(ws, base_row, ib, stat_key, stat_label, override_expr):
    ...
    for key, r in ROW.items():
        ...
        if key == "EMPOWERED_PIERCING_ARROW":
            ...
            ws.cell(row=row, column=15, value=(
                f'=IF(C{row},0.15*{bolt_surplus_trigger_rate_block}*{S("HitsPerCast", r)}*N{row}*'
                f"{target_multiplier_expr(ib('monster_type'), ib('normal_weight_frac'), S('NormalMonsterTargets', r), ib('max_enemies_hit'))},0)"
            ))
        else:
